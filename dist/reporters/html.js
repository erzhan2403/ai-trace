...
function generateHtmlReport(events, sessionFilePath) {
    const s = (0, console_js_1.buildSummary)(events);
    ...
    const timelineRows = events
        .filter((e) => [
        events_js_1.EventType.USER_INPUT,
        events_js_1.EventType.AGENT_STEP,
        events_js_1.EventType.LLM_CALL_START,
        events_js_1.EventType.LLM_CALL_END,
        events_js_1.EventType.TOOL_CALL_START,
        events_js_1.EventType.TOOL_CALL_END,
        events_js_1.EventType.TOOL_CALL_ERROR,
        events_js_1.EventType.FINAL_OUTPUT,
    ].includes(e.type))
        .map((e) => {
        const meta = e.metadata;
        const typeClass = e.type.toLowerCase().replace(/_/g, "-");
        const detail = meta
            ? truncate(meta.tool_name?.toString() ??
    ...
    const toolCallRows = events
        .filter((e) => e.type === events_js_1.EventType.TOOL_CALL_END || e.type === events_js_1.EventType.TOOL_CALL_ERROR)
        .map((e) => {
        const meta = e.metadata;
        const isError = e.type === events_js_1.EventType.TOOL_CALL_ERROR;
        ...
        const startEvent = events.find((s) => s.type === events_js_1.EventType.TOOL_CALL_START &&
            s.metadata.tool_name === meta.tool_name);
        const inputPreview = startEvent
            ? escapeHtml(truncate(JSON.stringify(startEvent.metadata.input ?? "")))
            : "";
        return `
        <tr>
    ...
    const llmRows = events
        .filter((e) => e.type === events_js_1.EventType.LLM_CALL_END)
        .map((e) => {
        const meta = e.metadata;
        return `
        <tr>
    ...
    // Warnings section
    const warningItems = [
        ...s.latencyWarnings.map((w) => `<li class="warning">⚠ ${w.type === "slow_tool" ? "Slow tool" : "Slow LLM"}: <strong>${escapeHtml(w.name)}</strong> took ${formatMs(w.duration_ms)} (threshold: ${formatMs(w.threshold_ms)})</li>`),
        ...s.loopWarnings.map((w) => `<li class="warning">⚠ Loop detected: <strong>${escapeHtml(w.tool_name)}</strong> called ${w.consecutive_count}× consecutively</li>`),
        ...s.errors.details.map((e) => `<li class="error-item">✗ Tool error: <strong>${escapeHtml(e.tool_name)}</strong> — ${escapeHtml(e.error_message)}</li>`),
    ].join("");
    const warningsSection = warningItems
